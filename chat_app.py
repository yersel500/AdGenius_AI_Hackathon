...
image_agent = Agent('groq:llama-3.3-70b-versatile',
                    system_prompt=(
...
async def generate_dalle_image(prompt: str, n: int = 1, size: str = "1024x1024") -> dict:
...
async def create_image_variation(image_path: str, n: int = 1, size: str = "1024x1024") -> dict:
    async with aiohttp.ClientSession() as session:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(original_image.image_url) as response:
            if response.status == 200:
                ...
                try:
                    result = await create_image_variation(str(image_path), request.n, request.size)
                    if 'data' in result:
...
async def post_chat(
    ...
    async def stream_messages():
        ...
        async with agent.run_stream(prompt, message_history=messages, deps=MyDeps(text_agent, image_agent)) as result:
